...
const authorCreate = (author) => {
  const createdAuthor = { name: author, id: uuid() }
  authors = authors.concat(createdAuthor)
...
const resolvers = {
  Query: {
    ...
    findAuthor: (root, args) => authorF(args.name),
    findBook: (root, args) => books.find((b) => b.name === args.name),
  },
  Book: {
    author: ({ author }) => {
      return authorFind(author)
        ? authorFind(author).name
        : authorCreate(author).name
    },
  },
  Author: {
    bookCount: ({ name }) => {
  ...
  Mutation: {
    addAuthor: (root, args) => {
      ...
      return authorCreate(args).name
    },

    editAuthor: (root, args) => {
      const author = authorFind(args.name)
      if (!author) {
        return null
      }
